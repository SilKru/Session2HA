---
title: "R Notebook"
output: html_notebook
---

```{r}
library(tidyverse)
```

```{r}
books <- BX_Books %>%
  select(ISBN,`Book-Title`,`Book-Author`,`Year-Of-Publication`,Publisher)
```


```{r}
users <- BX_Users %>%
  mutate(Age = as.numeric(Age))
```

```{r}
ratings <- BX_Book_Ratings %>%
  left_join(BX_Books)
```
1. Gibt es eine lineare Beziehung zwischen Anzahl Bücher pro Publisher und Anzahl Ratings, die die Bücher eines Publishers erhalten? (hier müssen Sie wieder etwas Data Wrangling betreiben)

-- Code funktioniert an sich, muss nur noch hübsch und ausformuliert werden 


```{r}
books_neu <- books %>%
  group_by(Publisher) %>%
  summarise(Anzahl = n()) %>%
  arrange(desc(Anzahl)) %>%
  filter(Anzahl > 199)

ratings_neu <- ratings %>%
  group_by(`Publisher`) %>%
  filter(`Book-Rating` > 0) %>%
  summarise(Durchschnittsbewertung = mean(`Book-Rating`), AnzahlBewertungen = n() ) %>%
  arrange(desc(Durchschnittsbewertung)) %>% 
  drop_na()
  #filter (AnzahlBewertungen >499)

publisher <- left_join(books_neu, ratings_neu, by="Publisher") #%>%
  #filter (AnzahlBewertungen>Anzahl) # weil es gab einen bzw. mehrere Ausreiser mit mehr Büchern als Bewertungen.. vermutlich viele Bücher ohne Bewertungen. So würden nur Verlage angezeigt, die mind so viele Bewertungen wie Bücher haben. macht R Wert besser aber keine Ahnung ob richitg . Vielleicht als Alternative einfügen

  

fit <- lm(publisher$Anzahl~publisher$AnzahlBewertungen)

plot(publisher$AnzahlBewertungen, publisher$Anzahl)

abline(fit)

summary(fit)


```

2. Haben ältere Bücher mehr Ratings, weil sie mehr Zeit haben, um Ratings zu sammeln? (muss nicht unbedingt mit einer linearen Regression analysiert werden)


```{r}
books_neu2 <- ratings %>%
  group_by(`Book-Title`, `Year-Of-Publication`) %>%
  summarise(AnzahlBewertungenBuch = n()) %>%
  arrange(desc(AnzahlBewertungenBuch))%>%
  drop_na()%>%
  filter(`Year-Of-Publication`> 1500)%>%
  filter(`Year-Of-Publication`< 2021)%>% 
  filter(AnzahlBewertungenBuch < 1000) #Ausreiser rausgefiltert, um Plot anschaulich zu machen
```


```{r}
fit2 <- lm(books_neu2$AnzahlBewertungenBuch~books_neu2$`Year-Of-Publication`)

(plot(books_neu2$`Year-Of-Publication`, books_neu2$AnzahlBewertungenBuch))

abline(fit2)

(summary(fit2))
```
  


```{r}
  ratings %>%
  filter(`Book-Rating` > 0) %>%
  count (Publisher, sort = TRUE, name="AnzahlBücher") #vermutlich falsch weil Bewertungen pro Verlag gezählt werden 
  #filter(`Publisher` != is.na) %>%   NAs noch wegmachen
```

```{r}

ratings %>%
  group_by(`Publisher`) %>%
  filter(`Book-Rating` > 0) %>%
  summarise(Durchschnittsbewertung = mean(`Book-Rating`), AnzahlBewertungen = n() ) %>%
  arrange(desc(Durchschnittsbewertung)) %>% 
  filter (AnzahlBewertungen >9)


# ich muss zählen, wie viele Bücher die Verlage jeweils publiziert haben 
#fit <- lm(ratings$AnzahlBewertungen~ratings$Publisher)
```


```{r}

```










Wer sind die häufigsten Publisher?

```{r}
books %>%
  group_by(Publisher) %>%
  summarise(Anzahl = n()) 
```
Wir haben hier sehr viele Publisher drin, die nur ein Buch veröffentlicht haben, diese sollten wir rausnehmen. Manche Publisher haben aber auch sehr seltsame Namen, so dass dies auch bereinigt werden sollte.

```{r}
books %>%
  group_by(Publisher) %>%
  summarise(Anzahl = n()) %>%
  arrange(desc(Anzahl)) %>%
  filter(Anzahl > 49)
```
```{r}
ratings %>%
  group_by(ISBN) %>%
  filter(`Book-Rating` > 0) %>%
  summarise(Durchschnittsbewertung = mean(`Book-Rating`), AnzahlBewertungen = n()) %>%
  arrange(desc(Durchschnittsbewertung))
```

Hausaufgabe: 

1. Welche Bücher sind am besten bewertet? und 2. Sollte die Anzahl der Bewertungen auch mit reinspielen? 

Zahlreiche Bücher haben ein Rating von 10. Dies zeigt die letzte Berechnung aus Ihrem Notebook. Jedoch haben die meisten davon ledigliche ein Bewertung. Ich finde um einen möglichst repräsentativen Wert zu erhalten, sollte eine Mindestanzahl an Bewertungen erreicht werden, um ins Besten-Ranking aufgenommen zu werden. Zum Beispiel mindestens 5 Ratings, in diesem Fall sind folgende ISBNs am besten bewertet. Damit fliegen auch die ungültigen/merkwürdigen ISBNs raus. 

```{r}
ratings %>%
  group_by(ISBN) %>%
  filter(`Book-Rating` > 0) %>%
  summarise(Durchschnittsbewertung = mean(`Book-Rating`), AnzahlBewertungen = n()) %>%
  arrange(desc(Durchschnittsbewertung)) %>%
  filter(AnzahlBewertungen > 4)
```
3. Welche Autoren sind am besten bewertet?

Auch bei den Autoren habe ich eine Mindestanzahl für Bewertungen gewähl, in diesem Fall 10. Damit werden Schreibfehler/andere merkwürdige Autorennamen aussortiert sowie ein repräsentatives Ergebnis erzielt. In diesem Fall haben die drei Autor*innen Michiro Ueyama, Pamela E. Apkarian-Russell und Wataru Yoshizumi mindestens 10 Bewertungen sowie jeweils die Bestnote von 10 bei jeder Bewertung. 

```{r}
ratings %>%
  group_by(`Book-Author`) %>%
  filter(`Book-Rating` > 0) %>%
  summarise(Durchschnittsbewertung = mean(`Book-Rating`), AnzahlBewertungen = n()) %>%
  arrange(desc(Durchschnittsbewertung)) %>% 
  filter (AnzahlBewertungen >9)
```

4. Welcher Verlag hat die besten Bücher?

Auch beim Verlag habe ich die Mindestanzahl auf 10 gesetzte. Die drei Verlage Collectors Press, Reagent Press	und Plan Nine Pub haben in diesem Fall die besten Bewertungen. 

```{r}
ratings %>%
  group_by(`Publisher`) %>%
  filter(`Book-Rating` > 0) %>%
  summarise(Durchschnittsbewertung = mean(`Book-Rating`), AnzahlBewertungen = n()) %>%
  arrange(desc(Durchschnittsbewertung)) %>% 
  filter (AnzahlBewertungen >9)
```
